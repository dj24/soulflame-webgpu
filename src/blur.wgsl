...
fn calculateRayDirection(uv: vec2<f32>, directions: FrustumCornerDirections) -> vec3<f32> {
    ...
    let finalInterpolated = mix(topInterpolated, bottomInterpolated, uv.y);
...
fn main(
  ...
  let p = (2.0*vec2<f32>(WorkGroupID.xy)-vec2<f32>(resolution)) / vec2<f32>(resolution).y;
  let rayOrigin = vec3(0,0,5.0);
  let rayDirection = normalize( vec3(p,-2.0) );
