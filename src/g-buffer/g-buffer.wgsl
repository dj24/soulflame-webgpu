...
@group(0) @binding(7) var velocityTex : texture_storage_2d<rgba8snorm, write>;
...
//  }
  ...
  var uv = vec2<f32>(GlobalInvocationID.xy) / vec2<f32>(resolution) * vec2(RAYS_PER_THREAD);
  uv = vec2(uv.x, 1.0 - uv.y);

  var pixel = GlobalInvocationID.xy * vec2(RAYS_PER_THREAD);
